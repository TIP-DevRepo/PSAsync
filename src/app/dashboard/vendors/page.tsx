...
const STATUS_COLORS: Record<string, string> = {
  ...
  PREFERRED: "bg-info-bg text-info",
}

type SortColumn = "name" | "type" | "email" | "phone" | "status"
...
function compareVendors(a: Vendor, b: Vendor, column: SortColumn): number {
  switch (column) {
    case "name":
...
export default function VendorsListPage() {
  ...
  const sorted =
    sortColumn && sortDirection
      ? [...filtered].sort((a, b) => {
          const cmp = compareVendors(a, b, sortColumn)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-display font-semibold tracking-tight text-foreground">Vendors</h1>
              ...
              >
                <td className={`${ROW_PADDING[density]} px-3`}>
                  ...
                  {vendor.isDistributor && (
                    <span className="ml-2 rounded-full bg-brand-secondary-500/10 px-2 py-0.5 text-xs font-medium text-brand-secondary-500">
                      Distributor
                    </span>
                  )}
                </td>
